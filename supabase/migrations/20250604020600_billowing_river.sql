/*
  # Security Enhancements for Trivia App

  1. Enable RLS
    - Enable row level security on all tables that have policies
  2. Fix Policies
    - Update game_sessions policies with proper permissions for mobile controller
  3. Secure Functions
    - Create SECURITY DEFINER versions of critical functions
    - Add proper search_path settings for security
  4. Realtime Updates
    - Add poll_votes to realtime publication for live updates
*/
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Public can view and manage game sessions" ON public.game_sessions;
  ...
  -- Only create this policy if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Public can view and manage game sessions' 
    AND polrelid = 'public.game_sessions'::regclass
  ) THEN
...
-- Create apply_room_theme_to_templates function if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
...
DO $$
BEGIN
  -- Check if poll_votes is already in the publication
  IF NOT EXISTS (
