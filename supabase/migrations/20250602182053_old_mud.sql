/*
  # Room Code Management Improvements
  
  1. Changes
     - Updates existing rooms with invalid room codes to have proper 4-character codes
     - Improves the room code generation function to ensure uniqueness and proper format
     - Adds a constraint to enforce 4-character room codes
     
  2. Error Handling
     - Properly handles dependencies between triggers and functions
     - Uses CASCADE where needed to avoid dependency errors
*/
...
-- Then drop the function (now safe since the trigger is gone)
