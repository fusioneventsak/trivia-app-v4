/*
  # Fix RLS and Security Settings

  1. New Functions
    - Enable RLS on all tables with policies
    - Create secure versions of existing functions with SECURITY DEFINER
    - Add new function to apply room themes to templates
    - Fix game session policies for mobile controller access

  2. Security
    - All new functions use SECURITY DEFINER
    - All new functions set search_path to public
    - Game sessions now allow public access for mobile controllers
*/
...
-- Create a function to apply room theme to templates (check if it exists first)
DO $$
BEGIN
  IF NOT EXISTS (
