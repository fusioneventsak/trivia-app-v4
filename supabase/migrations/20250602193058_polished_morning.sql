/*
  ...
  1. New Columns
    - Add `option_id` column to `poll_votes` table to store the unique ID of the selected option
  
  2. Changes
    - Rename existing `answer` column to `option_text` for clarity and consistency
    - Create function to ensure all options in activations have unique IDs
    - Update existing poll votes to use option IDs where possible
  
  3. Security
    - Maintain existing RLS policies
*/

-- Add option_id column to poll_votes table if it doesn't exist
