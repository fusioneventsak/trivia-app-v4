/*
  # Fix RLS and Function Security

  1. New Security Features
    - Enable Row Level Security (RLS) on all tables that need it
    - Add SECURITY DEFINER to functions to bypass RLS when needed
  ...
  2. Changes
    ...
    - Update functions to use SECURITY DEFINER with proper search_path
    - Create new versions of functions without dropping existing ones
  
  3. Security
    - Improve RLS policies for game_sessions
    - Fix function security to prevent privilege escalation
*/
...
-- Create a function to apply room theme to templates
CREATE OR REPLACE FUNCTION public.apply_room_theme_to_templates(p_room_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
