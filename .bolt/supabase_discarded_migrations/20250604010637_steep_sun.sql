/*
  ...
  1. Changes
     ...
     - Add SECURITY DEFINER to functions to bypass RLS
     - Set search_path for functions to fix security warnings
     
  2. Security
     - Properly secure functions with SECURITY DEFINER
     - Fix RLS policies for game_sessions to allow public access
*/
...
-- First drop the trigger that depends on it, then recreate both
DO $$
BEGIN
  -- Drop the trigger first
  DROP TRIGGER IF EXISTS ensure_game_session_trigger ON public.activations;
  
  -- Now we can safely drop and recreate the function
  DROP FUNCTION IF EXISTS public.ensure_game_session();
  ...
  EXECUTE FUNCTION public.ensure_game_session();
END
$$;

-- Fix validate_activation function
DROP FUNCTION IF EXISTS public.validate_activation(uuid);
...
DROP FUNCTION IF EXISTS public.check_cross_room_compatibility(uuid);
...
-- Drop and recreate get_poll_votes function
DO $$
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    RAISE NOTICE 'Error dropping function get_poll_votes: %', SQLERRM;
END
$$;
...
DROP FUNCTION IF EXISTS public.has_player_voted(uuid, uuid);
...
DROP FUNCTION IF EXISTS public.update_room_theme(uuid, jsonb);
...
-- Fix set_room_code function - handle trigger dependency
DO $$
BEGIN
  -- Drop the trigger first if it exists
  DROP TRIGGER IF EXISTS ensure_room_code ON public.rooms;
  
  -- Now drop and recreate the function
  DROP FUNCTION IF EXISTS public.set_room_code();
...
DROP FUNCTION IF EXISTS public.fix_player_stats();
...
-- Fix generate_option_ids function - handle trigger dependency
DO $$
BEGIN
  -- Drop the trigger first if it exists
  DROP TRIGGER IF EXISTS ensure_option_ids ON public.activations;
  
  -- Now drop and recreate the function
  DROP FUNCTION IF EXISTS public.generate_option_ids();
  ...
  BEFORE INSERT OR UPDATE ON public.activations
  FOR EACH ROW
  EXECUTE FUNCTION public.generate_option_ids();
END
$$;
...
DROP FUNCTION IF EXISTS public.check_options_format(jsonb);
